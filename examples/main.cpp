...
int main()
{
  ...
  vcan0_network->insert(addr_mngr);
  vcan0_network->set_callback([](jay::name name, uint8_t address) -> void {
  ...
  conn->on_read([vcan0_network](auto frame) { vcan0_network->process(frame); });
