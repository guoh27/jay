...
public:
   ...
   */
  name(std::uint32_t id_num,
    std::uint16_t man_code,
    std::uint8_t ecu_inst,
    std::uint8_t func_inst,
    std::uint8_t func,
    std::uint8_t dev_cls,
    std::uint8_t dev_cls_inst,
    std::uint8_t ind_grp,
    std::uint8_t slf_cfg_addr)
    : name_(((static_cast<name_t>(slf_cfg_addr) << slf_cfg_addr_start_bit) & slf_cfg_addr_mask)
  ...
  */
  ...
  /**
   * @brief Check if this name has a higher priority than the another name
   *
   * Helper functions so that we dont have to remember the comparison order for higher priority.
   * The lowest NAME value will have the highest priority.
   *
   * @param other name to compare with
   * @return true if this name has higher priority
   * @return false if the other name has higher priority
   */
  [[nodiscard]] inline bool has_priority_over(const jay::name other) const noexcept { return name_ < other.name_; }
   ...
   */
  inline name &identity_number(const std::uint32_t id_num) noexcept
  {
    //             Combine
    // Clear existing | ((place bits correctly) & remove extra bits)
    name_ = (name_ & ~id_num_mask) | (static_cast<name_t>(id_num << id_num_start_bit) & id_num_mask);
    return *this;
  }
   ...
   * @return reference to self
   */
  inline name &manufacturer_code(const std::uint16_t man_code)
  {
    name_ = (name_ & ~man_code_mask) | ((static_cast<name_t>(man_code) << man_code_start_bit) & man_code_mask);
    return *this;
  }
   ...
   * @return reference to self
   */
  inline name &ecu_instance(const std::uint8_t ecu_inst) noexcept
  {
    name_ = (name_ & ~ecu_inst_mask) | ((static_cast<name_t>(ecu_inst) << ecu_inst_start_bit) & ecu_inst_mask);
    return *this;
  }
   ...
   * @return reference to self
   */
  inline name &function_instance(const std::uint8_t func_inst) noexcept
  {
    name_ = (name_ & ~func_inst_mask) | ((static_cast<name_t>(func_inst) << func_inst_start_bit) & func_inst_mask);
    return *this;
  }
   ...
   * @return reference to self
   */
  inline name &function(const std::uint8_t func) noexcept
  {
    name_ = (name_ & ~func_mask) | ((static_cast<name_t>(func) << func_start_bit) & func_mask);
    return *this;
  }
   ...
   * @return reference to self
   */
  inline name &device_class(const std::uint8_t dev_cls) noexcept
  {
    name_ = (name_ & ~dev_cls_mask) | ((static_cast<name_t>(dev_cls) << dev_cls_start_bit) & dev_cls_mask);
    return *this;
  }
   ...
   * @return reference to self
   */
  inline name &device_class_instace(const std::uint8_t dev_cls_inst) noexcept
  {
   ...
   * @return reference to self
   */
  inline name &industry_group(const std::uint8_t ind_grp) noexcept
  {
    name_ = (name_ & ~ind_grp_mask) | ((static_cast<name_t>(ind_grp) << ind_grp_start_bit) & ind_grp_mask);
    return *this;
  }
   ...
   * @return reference to self
   */
  inline name &self_config_address(const std::uint8_t slf_cfg_addr) noexcept
  {
   ...
   */
  [[nodiscard]] inline std::uint32_t identity_number() const noexcept
  {
   ...
   */
  [[nodiscard]] inline std::uint16_t manufacturer_code() const noexcept
  {
   ...
   */
  [[nodiscard]] inline std::uint8_t ecu_instance() const noexcept
  {
   ...
   */
  [[nodiscard]] inline std::uint8_t function_instance() const noexcept
  {
   ...
   */
  [[nodiscard]] inline std::uint8_t function() const noexcept
  {
   ...
   */
  [[nodiscard]] inline std::uint8_t device_class() const noexcept
  {
   ...
   */
  [[nodiscard]] inline std::uint8_t device_class_instace() const noexcept
  {
   ...
   */
  [[nodiscard]] inline std::uint8_t industry_group() const noexcept
  {
   ...
   */
  [[nodiscard]] inline std::uint8_t self_config_address() const noexcept
  {
  ...
  inline bool operator<(const name &rhs) const noexcept { return (name_ < rhs.name_); }

  inline bool operator>(const name &rhs) const noexcept { return (name_ > rhs.name_); }

  inline bool operator==(const name &rhs) const noexcept { return (name_ == rhs.name_); }

  inline bool operator<(const name_t rhs) const noexcept { return (name_ < rhs); }
  ...
  struct hash
  {
    inline size_t operator()(const jay::name &name) const { return std::hash<name_t>()(name.name_); }
  };
   ...
   */
  inline operator name_t() const noexcept { return name_; }
