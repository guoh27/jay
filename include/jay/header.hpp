...
public:
  ...
  */
  frame_header(const priority_t priority, const bool data_page, 
    const std::uint8_t pdu_format, const std::uint8_t pdu_specific, 
    const std::uint8_t source_address, const std::size_t payload = 0) : 
    frame_header(
      priority,
      (static_cast<std::uint32_t>(data_page)  << 16) |
      (static_cast<std::uint32_t>(pdu_format) << 8)  |
   ...
   * @param source_address of the frame
  */
  frame_header(const priority_t priority, const pgn_t pgn, 
    const std::uint8_t source_address, const std::size_t payload = 0) : 
  frame_header(
      (std::clamp(static_cast<std::uint32_t>(priority), 0U, 7U) << 26) |
  ...
  */
  priority_t priority() const noexcept
  {
  ...
  */
  std::uint8_t data_page() const noexcept
  {
  ...
  */
  ...
  {
    return (pgn() & J1939_PGN_PDU1_MAX) == J1939_PGN_REQUEST;
  }
...
static_assert(sizeof(jay::frame_header) == sizeof(std::uint64_t),
