...
public:
   ...
   */
  void insert(jay::address_manager& addr_man)
  {
    name_manager_map.insert({addr_man.get_name(), addr_man});
  }
  ...
  */
  void process(const jay::frame& frame)
  {
    ...
    {
      on_frame_address_claim(jay::name(frame.payload), 
        frame.header.pdu_specific(), frame.header.source_adderess());
      return;
    }

    if(frame.header.is_request())
    {
      on_frame_address_request(frame.header.pdu_specific());
    }
  }
...
private:
  ...
  void on_frame_address_claim(jay::name name, std::uint8_t pdu_specific, std::uint8_t source_adderess)
  {
    ...
    {
      if(auto name = network_.get_name(pdu_specific); name.has_value())
      { //Claim targeted at specific address
    ...
    for(auto& ctrl : name_manager_map)
    {
      ctrl.second.address_claim(claim);
    }
  }
  ...
  void on_frame_address_request(std::uint8_t address)
  {
    ...
    {
      if(auto name = network_.get_name(address); name.has_value())
      { //Request address claim from specific address
        if(auto it = name_manager_map.find(name.value()); it != name_manager_map.end()) 
        { 
    ...
    for(auto& ctrl : name_manager_map)
    {
      ctrl.second.address_request(req);
    }
  }
